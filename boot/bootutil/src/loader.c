...
find_slot_not_tested(struct boot_loader_state *state)
{
    ...
    for (slot = 0; slot < BOOT_NUM_SLOTS; slot++) {
		...
		BOOT_LOG_INF("active_swap_state k_malloc assert");
		assert(active_swap_state != 0);
		BOOT_LOG_INF("active_swap_state k_malloc work");

		rc = boot_read_swap_state(fap, active_swap_state);
		assert(rc == 0);
		
		BOOT_LOG_INF("Image slot %d, flag %d", slot, active_swap_state->copy_done);
